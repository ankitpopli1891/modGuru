...
public class SupplicantBroadcast extends BroadcastReceiver {
	...
	private void changeState(DetailedState aState) 
	{
			...
			new Thread(){
				public void run() {
					do
					{
						try{
							Thread.sleep(1000);
							broadcastMessage.sendMessage("[sendQuiz"+serverIP+"]", mtu, 5573, serverIP.getHostAddress());
							broadcastMessage.receiveMessage(5000);
						}
						catch (Exception e) {
						}
					}while(!BroadcastMessageReceiver.ackQuiz);
					...
					try{
						new Handler(context.getMainLooper()).post(new Runnable() {
							...
							@Override
							public void run() {
								new Client(context).execute(serverIP.getHostAddress(), "5571");
							}
						});
					}
					catch (Exception e) {
		...
		else if (aState == DetailedState.DISCONNECTED)
		{
			connected=-1;
			Log.d("Wi-Net", "Connection State Change:"+aState);
//			WiNetService.peerBroadcast.disconnect();
//			WiNetService.broadcast.disconnect();
//			WiNetService.ipAddr=null;
			broadcastMessage.disconnect();
			broadcastMessage=null;
//			WiNetService.info=null;
		}
	...
	public void checkState(int aInt)
	{
		...
		else if (aInt == 11)
		{
			apEnabled=-1;
			if(wifiEnabled==-1){
//			if(WiNetService.peerBroadcast!=null)			
//				WiNetService.peerBroadcast.disconnect();
//			if(WiNetService.broadcast!=null){
//				WiNetService.broadcast.disconnect();			
//				WiNetService.ipAddr=null;
//				WiNetService.broadcast=null;
//				WiNetService.info=null;
//			}
				server.destroy();
				server=null;
		...
		else if (aInt == 13)
		{
			...
			new Thread(){
				public void run() {
					...
					server=new Server(context, 5571);
					ipAddr=getLocalIpAddress();
					apEnabled=1;
					new Thread(){
						public void run() {
